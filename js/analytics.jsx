...
const AnalyticsDashboard = () => {
    ...
    const processed = useMemo(() => {
        ...
        const totalLabor = laborCommission + laborSalary + supervisorSalesCommTotal;
        ...
        const chartArqueos = fCashCounts
            ...
            .map(c => ({
                ...
                diff: c.diffMxn + (c.diffUsd * (c.exchangeRate || 18)), 
                user: c.user
            }));
        ...
        fTickets.forEach(t => {
            ...
            if (!p) {
                payments['Efectivo MXN'] += t.price || 0;
                return;
            }

            if (p.method === 'CARD') {
                payments['Tarjeta'] += t.price;
            } else {
                if (p.isOnlyUsd) {
                    payments['Efectivo USD'] += ((p.usd || 0) - (p.changeUsd || 0)) * (p.rate || 18);
                } else {
                    payments['Efectivo MXN'] += (p.mxn || 0) - (p.changeMxn || 0);
                    payments['Efectivo USD'] += (p.usd || 0) * (p.rate || 18);
                }
            }
        });

        const chartPayments = Object.keys(payments)
            .map(k => ({ name: k, value: payments[k] }))
            .filter(item => item.value > 0);

        const totalPaymentFlow = chartPayments.reduce((acc, item) => acc + item.value, 0);
        ...
        return {
            ...
            avgTicketsPerDay,
            laborCommission, laborSalary, supervisorSalesCommTotal, totalLabor,
            laborCommPct, laborSalaryPct, laborSupCommPct, totalLaborPct,
            totalOpExpenses, totalOpExpensesPct,
            armorAllPerWash, totalArmorAllCost, salariedCount,
            chartFinancials, chartExpenses, chartArqueos, chartPayments, totalPaymentFlow, chartEmployees, chartServices, totalShortage
        };
        ...
        <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                    <KPICard title="Ingreso Bruto" value={formatCurrency(processed.totalRevenue)} subtext={`${processed.ticketCount} vehículos`} icon={DollarSign} color="blue" />
                    <KPICard title="Nómina" value={formatCurrency(processed.totalLabor)} subtext={`${processed.totalLaborPct.toFixed(1)}% de Ingreso`} icon={Users} color="orange" />
                    <KPICard title="Gastos Operativos" value={formatCurrency(processed.totalOpExpenses)} subtext={`${processed.totalOpExpensesPct.toFixed(1)}% de Ingreso`} icon={ArrowDownRight} color="red" />
                    <KPICard title="Utilidad Neta" value={formatCurrency(processed.netProfit)} subtext={`${processed.profitMarginPct.toFixed(1)}% de Ingreso`} icon={Wallet} color={processed.netProfit > 0 ? "green" : "red"} />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <KPICard title="Ticket Promedio" value={formatCurrency(processed.avgTicket)} subtext="Ingreso por auto" icon={TrendingUp} color="purple" />
                    <KPICard title="Promedio Autos/Día" value={processed.avgTicketsPerDay.toFixed(1)} subtext="Vehículos diarios" icon={Car} color="cyan" />
                    <KPICard title="ArmorAll / Lavado" value={formatCurrency(processed.armorAllPerWash)} subtext={`Gasto Total: ${formatCurrency(processed.totalArmorAllCost)}`} icon={Droplets} color="indigo" />
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className={`bg-white p-6 rounded-xl shadow-sm border h-96 ${timeRange === 'MONTH' ? 'lg:col-span-3' : ''}`}>
                        ...
                        <ResponsiveContainer width="100%" height="80%">
                            <PieChart>
                                <Pie data={processed.chartExpenses} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
            ...
            <div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm border h-80">
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={processed.chartPayments} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                                    {processed.chartPayments.map((entry, index) => (
                                        <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                                    ))}
                                </Pie>
                                <Tooltip formatter={(val) => `${((val / processed.totalPaymentFlow) * 100).toFixed(1)}%`} />
